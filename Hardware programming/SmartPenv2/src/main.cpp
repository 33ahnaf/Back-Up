...
void setup() {
    ...
    penButton.begin(Config::PIN_PEN_BUTTON, Config::BUTTON_DEBOUNCE_MS);
    ...
    Serial.println(F("[main] Ready. Pen-tip button on D7 controls pen "
                      "up/down. Serial commands: c=calibrate, r=reset "
                      "position, d=toggle verbose debug."));
}
...
void loop() {
    ...
    // Polled every iteration (not gated behind the IMU sample-ready check
    // below) so button edges are never missed and debounce timing stays
    // accurate regardless of IMU timing.
    penButton.update();
    ...
    bool haveNewSample = tracker.update(penButton.isPressed());
