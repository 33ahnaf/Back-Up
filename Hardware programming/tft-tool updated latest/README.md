...
This mode is interactive rather than automatic: on boot it asks over
Serial which permutation index to start checking from (just press Enter
for 0), then after testing each permutation it waits for you to send `n`
before moving on to the next one. That gives you time to actually look at
the panel after each permutation instead of it flashing past unattended.
See "Pin permutation stepping" below.
...
## Pin permutation stepping

With `ENABLE_PIN_PERMUTATION_TEST` on, open the serial monitor (115200
baud, newline line-ending is fine) right after flashing:

1. The tool prints the total permutation count and asks:
   `Enter permutation index to start at (0-119), or press Enter for 0:`
   Type a number and hit Enter to jump straight to that permutation, or
   just hit Enter to start from 0.
2. It applies that permutation, prints the `PIN PERMUTATION k / total`
   banner with the CS/DC/RST/MOSI/SCK mapping, and runs one full
   driver/rotation/color-order/invert/color sweep so you can watch the
   panel.
3. It then prints `Send 'n' to advance to the next permutation...` and
   waits. Send `n` (case-insensitive; a trailing Enter is fine) to move to
   permutation `k+1` and repeat from step 2. Anything else you send while
   it's waiting is ignored. After the last permutation it wraps back to 0.
...
There is no MISO line to read status back from, so a missing/miswired
panel simply results in "nothing visibly happens" for that driver/
combination rather than a hang or crash, and the sweep moves on to the
next combination/permutation. The only intentional blocking points are the
`ENABLE_PIN_PERMUTATION_TEST` prompts described above (start index, and
waiting for `n`) - with that flag off, the loop never blocks on user input
and runs fully unattended, as before.
