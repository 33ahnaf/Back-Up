...
bool Tracker::update() {
    ...
    // Synthetic pen-down/up: since there's no physical tip-contact switch
    // on IMU-only hardware, we treat "not stationary" as "actively
    // writing" and "stationary" as "pen lifted" — this gives the browser
    // a way to start a new stroke instead of drawing one giant connected
    // scribble, which is the best proxy available without extra hardware.
    frame_.penDown = isStationary_ ? 0 : 1;
    frame_.penDown = isPenDown();
...
bool Tracker::isPenDown(){
    static uint32_t lastChange = 0;
    static bool state = false;

    bool reading =
        (digitalRead(Config::PIN_BUTTON) == LOW);
